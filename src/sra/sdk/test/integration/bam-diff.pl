...
# start logging
open ( $log, ">", "bam-diff.$$.log" ) or die "Cannot open bam-diff.log for output";

my @required_directories = ( "samtools",
                             "vdbtools",
                             "samtools/aligned",
                             "vdbtools/aligned",
                             "samtools/unaligned",
...
foreach (@required_directories)
{
  if ( ! -d $_ )
  {
    ...
    print { $log } ( "Missing directory '$indir/$_', aborting...\n\n" );
    close ( $log );
    exit (3 );
  }
}
...
{
    print "samtool_aligned_core_files $#samtool_aligned_core_files\nvdb_aligned_core_files $#vdb_aligned_core_files\nsamtool_aligned_ext_files $#samtool_aligned_ext_files\nvdb_aligned_ext_files  $#vdb_aligned_ext_files\nsamtool_unaligned_files $#samtool_unaligned_files\nvdb_unaligned_files $#vdb_unaligned_files\n";
    log_it "samtool_aligned_core_files $#samtool_aligned_core_files\nvdb_aligned_core_files $#vdb_aligned_core_files\nsamtool_aligned_ext_files $#samtool_aligned_ext_files\nvdb_aligned_ext_files  $#vdb_aligned_ext_files\nsamtool_unaligned_files $#samtool_unaligned_files\nvdb_unaligned_files $#vdb_unaligned_files\n";
    log_it "Bad dump files - counts mismatch\n";
    close ( $log );
    exit ( 3 );
}
...
foreach $file ( @samtool_aligned_core_files )
{
  if ( ! -f "vdbtools/$file" )
  {
    log_it ( "ERROR: missing file 'vdbtools/$file'\n" );
    ++$error_count;
  }
}
...
foreach $file ( @samtool_unaligned_files )
{
  if ( ! -f "vdbtools/$file" )
  {
    log_it ( "ERROR: missing file 'vdbtools/$file'\n" );
...
foreach $file ( @vdb_aligned_core_files )
{
  if ( ! -f "samtools/$file" )
  {
    log_it ( "ERROR: missing file 'vdbtools/$file'\n" );
...
foreach $file ( @vdb_unaligned_files )
{
  if ( ! -f "samtools/$file" )
  {
    log_it ( "ERROR: missing file 'vdbtools/$file'\n" );
...
foreach $file ( @samtool_aligned_core_files )
{
  if ( -f "vdbtools/$file" )
  {
    ...
    if ( system ( "diff samtools/$file vdbtools/$file >$diffdir/$file.diffs" ) == 0 )
    {
       print {STDOUT} ( "files match '$file'\n" ) if ( $debug );
       log_it ( "files match '$file'\n" );
       unlink ( "$diffdir/$file.diffs" );
    }
    else
    {
       print {STDOUT} ( "ERROR: files differ see $diffdir/$file'\n" ) if ( $debug );
       log_it ( "ERROR: files differ see $diffdir/$file'\n" );
       ++$error_count;
    }
  }
}
...
foreach $file ( @samtool_aligned_ext_files )
{
  if ( -f "vdbtools/$file" )
  {
    ...
    if ( system ( "ext_diff.pl samtools/$file vdbtools/$file >$diffdir/$file.diffs" ) == 0 )
    {
      print {STDOUT} ( "files match '$file'\n" ) if ( $debug );
      log_it ( "files match '$file'\n" );
      unlink ( "$diffdir/$file.diffs" );
    ...
    {
      print {STDOUT} ( "ERROR: files differ see $diffdir/$file.diffs'\n" ) if ( $debug );
      log_it ( "ERROR: files differ see $diffdir/$file.diffs'\n" );
      ++$error_count;
    }
  }
}
...
if ( $error_count > 0 )
{
    print {STDOUT} ( "ERROR: directories differ\n" ) if ( $debug );
    log_it ( "ERROR: directories differ\n" );
}
