...
rc_t get_password (const VPath * path, char * pw, size_t pwz, size_t * num_read, KKey * key, bool * read)
{
    ...
    if (VPathOption (path, vpopt_encrypted, obuff, sizeof obuff, &z) == 0)
    {
        if (VPathOption (path, vpopt_pwpath, obuff, sizeof obuff, &z) == 0)
            rc = KDirectoryOpenFileRead (cwd, &pwfile, obuff);
                  ...
                  enum CCType ntype, CCFileNode *node, const char *name )
{
    ...
    /* create path */
    char path [ 256 ];
    int len = snprintf ( path, sizeof path, name );
    ...
    rc = KDirectoryCreateFile ( cdir, & out, false, 0640, cm, path );
    if ( rc != 0 && GetRCState ( rc ) == rcUnauthorized )
    {
        /* respond to a file that has no write privs */
        uint32_t access;
        rc_t rc2 = KDirectoryAccess ( cdir, & access, path );
        if ( rc2 == 0 )
        {
            rc2 = KDirectorySetAccess ( cdir, false, 0640, 0777, path );
            if ( rc2 == 0 )
            {
                rc = KDirectoryCreateFile ( cdir, & out, false, 0640, cm, path );
                if ( rc != 0 )
                    KDirectorySetAccess ( cdir, false, access, 0777, path );
            }
        }
    }
    ...
    else
    {
        ...
        if ( rc == 0 )
        {
            ...
            {
                rc = KDirectorySetAccess ( cdir, false, 0440, 0777, path );
...
rc_t ccat_extract (const KFile *sf, const char * path)
{
    ...
    rc = KDirectoryCreateFile ( edir, & out, false, 0640, cm, path );
    if ( rc != 0 && GetRCState ( rc ) == rcUnauthorized )
    {
        /* respond to a file that has no write privs */
        uint32_t access;
        rc_t rc2 = KDirectoryAccess ( edir, & access, path );
        if ( rc2 == 0 )
        {
            rc2 = KDirectorySetAccess ( edir, false, 0640, 0777, path );
            if ( rc2 == 0 )
            {
                rc = KDirectoryCreateFile ( edir, & out, false, 0640, cm, path );
                ...
                DBG_KFile(out);
                if ( rc != 0 )
                    KDirectorySetAccess ( edir, false, access, 0777, path );
            }
        }
    }
    if ( rc != 0 )
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                const char *ext = strrchr ( name, '.' );
                if ( ext == NULL )
                    len = snprintf ( newname, sizeof newname, name );
                else
