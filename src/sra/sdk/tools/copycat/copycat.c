    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    rc = KDirectoryDate (cwd, &mtime, spath);
                    if (rc)
                    {
                        PLOGERR (klogErr,
                                 (klogErr, rc,
                                ...
                                else
                                {
                                    ...
                                    if (rc == 0)
                                    {
                                        ...
                                        if (GetRCState (rc) == rcUnauthorized)
                                        {
                                            ...
                                            orc = KDirectoryAccess (cwd, &access, dpath);
                                            if (orc == 0)
                                            {
                                                orc = KDirectorySetAccess (cwd, false, 0640, 0777, dpath);
                                                if (orc == 0)
                                                {
                                                    rc = VFSManagerCreateFile (mgr, &df, false, 0640, cm, dst);
                                                    if (rc)
                                                        KDirectorySetAccess (cwd, false, access, 0777, dpath);
                                                }
                                            }
                                        }
                                        if (rc)
                   ...
                   Vector * v)
{
    ...
    if ( cache != NULL )
    {
        rc = KDirectoryCreateDir ( cwd, 0775, kcmParents | kcmOpen, cache );
        if ( rc == 0 )
            rc = KDirectoryOpenDirUpdate ( cwd, & cdir, true, cache );
        if ( rc != 0 )
        {
            PLOGERR (klogErr,
    ...
    if ( extract != NULL )
    {
        rc = KDirectoryCreateDir (cwd, 0775, kcmParents | kcmOpen, extract);
        if ( rc == 0 )
            rc = KDirectoryOpenDirUpdate (cwd, & edir, true, extract);
        if ( rc != 0 )
        {
            PLOGERR (klogErr, 
                     (klogErr, rc,
    ...
    dest_type = KDirectoryPathType (cwd, pbuff);
...
/* )) */
        ...
        rc = KDirectoryCreateDir ( cwd, 0775, kcmParents | kcmOpen, pbuff );
        if ( rc != 0 )
            return rc;
