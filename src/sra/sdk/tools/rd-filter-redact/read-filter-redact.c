    ...
    const Db* db, SpotIterator* self)
{
    ...
    if (rc == 0) {
        ...
        rc = KDirectoryOpenFileRead
            (__SpotIteratorDirectory, &self->file, self->filename);
        if (rc != 0) {
            PLOGERR(klogErr, (klogErr, rc,
...
static rc_t DbInit(rc_t rc, const CmdLine* args, Db* db)
{
    ...
    if( rc == 0) {
        ...
        if (rc == 0) {
            rc = VCursorAddColumn(db->rCursor, &db->rFilterIdx, name);
            if (rc != 0) {
                PLOGERR(klogErr, (klogErr, rc,
    ...
    if (rc == 0) {
        ...
        if (rc == 0) {
            rc = VCursorAddColumn(db->wCursor, &db->wIdx, name);
            if (rc != 0) {
                PLOGERR(klogErr, (klogErr, rc,
...
static rc_t DbDestroy(Db* db)
{
    ...
    if (db->locked) {
        rc_t rc2 = VDBManagerLock(db->mgr, db->table);
        if (rc == 0)
        {   rc = rc2; }
    }
