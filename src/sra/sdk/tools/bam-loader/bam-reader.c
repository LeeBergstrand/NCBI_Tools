...
rc_t BamReaderFileGetRecord ( const BamReaderFile *f, const Record** result )
{
    ...
    rc = BAMFileRead(self->reader, &record->seq.bam);
    if (rc)
    {
...
rc_t CC BamReaderFileMake( const ReaderFile **reader, char const headerText[], char const path[])
{
    ...
    else
    {
        ...
        rc = BAMFileMakeWithHeader ( &self->reader, headerText, self->dad.pathname );
                    ...
                    char const path[] )
{
    ...
    else
    {
        ...
        rc = BAMFileMakeWithHeader( & self->file, headerText, path);
        if ( rc != 0 )
        {
            free(self);
            *result = 0;
