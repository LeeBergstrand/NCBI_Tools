...
static rc_t DbConstruct(Db* db, CmdArgs* args) {
    ...
    while (rc == 0) {
        ...
        if (param == NULL)
        {   break; }
        rc = VSchemaParseFile(db->schema, param);
...
static rc_t DbUpdatePlatform(Db* self, const CmdArgs* args)
{
    ...
    assert(self && args);
    if (rc == 0) {
        rc = KMetadataOpenNodeRead(self->meta, &rNode, path);
    ...
    if (toUpdate) {
        if (rc == 0) {
            rc = KMetadataOpenNodeUpdate(self->meta, &uNode, path);
    ...
    if (metaUpdated) {
        ...
        if (rc == 0) {
            rc = VDBManagerCreateTable(self->mgr,
                &self->tbl, self->schema, args->schema, kcmOpen, args->run);
