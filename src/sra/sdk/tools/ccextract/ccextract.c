...
/*  ----------------------------------------------------------------------
 */
void extnode_whack (BSTNode * n, void * data)
{
    if (n)
    {
...
/*             KOutMsg ("%s: %s %x\n", __func__, node->path, options.cm); */
            rc = KDirectoryCreateFile (options.dir, &dfile, false, 0640, options.cm, buff);
            if (rc)
...
/*  ----------------------------------------------------------------------
 */
int sort_pathpath_cmp (const void ** litem, const void ** ritem, void * data)
{
    uint64_t lloc, rloc;

    {
        ...
        if (rc == 0)
        {
            switch (KDirectoryPathType (options.base, pbuff))
            {
            default:
                lloc = 0;
                break;
            case kptFile:
                rc = KDirectoryFileLocator (options.base, &lloc, pbuff);
                break;
            }
            if (rc == 0)
            {
                ...
                if (rc == 0)
                {
                    ...
                    default:
                        ...
                        break;
                    case kptFile:
                        rc = KDirectoryFileLocator (options.base, &rloc, pbuff);
                        break;
                    }
                }
            }
...
rc_t walk_path_file (char * path, size_t z, uint64_t * offset, KPathType kpt)
{
    ...
    if (kpt == kptFile)
    {
        rc = KDirectoryFileLocator (options.base, &this_offset, path);
        if (rc)
            PLOGERR (klogErr,
    ...
    if (rc == 0)
    {
        ...
        if (pc)
        {
            ...
            *pc = '\0';
            lkpt = KDirectoryPathType (options.base, path);
...
/*         char tbuff [8193]; */
        ...
        else if (z)
        {
            ...
            kpt = KDirectoryPathType (options.base, pbuff);
...
/* ain't doing this now, and may never because of the root versus base problems */
            ...
            while (kpt & kptAlias)
            {
                ...
                else
                {
                    ...
                    z = string_size (pbuff);
                    kpt = KDirectoryPathType (options.root, pbuff);
                }
            }
#endif
...
/*  ----------------------------------------------------------------------
 */
void build_vpath_one (void * item, void * data)
{
    const char * path;
    rc_data * prc;
    ...
    if (prc->rc == 0)
    {
            ...
            (0 == string_cmp (path, (hier+1) - path, ccid, cz /*sizeof (ccid) - 1*/, cz)))
        {
            ...
            rc = KDirectoryResolveAlias (options.root, true, rbuff, sizeof (rbuff), ibuff);
...
/*  ----------------------------------------------------------------------
 */
void build_vpath_whack (void * item, void * data)
{
    VPath * p;
...
rc_t open_root_then_run ()
{
    ...
    }
    {
        ...
        else
        {
            ...
            {
                rc = VPathMake (&options.basepath, options.basestr);
                if (rc)
                    PLOGERR (klogErr, (klogErr, rc,
...
rc_t open_xml_then_run()
{
    ...
    rc = VPathMake (&options.xmlpath, options.xmlstr);
    if (rc)
        PLOGERR (klogInt,
...
rc_t open_dir_then_run()
{
    ...
    rc = VPathMake (&options.dirpath, options.dirstr);
    if (rc)
        PLOGERR (klogInt,
                 (klogInt, rc, "failed to create path for '$(P)'",
                  "P=%s", options.dirstr));
