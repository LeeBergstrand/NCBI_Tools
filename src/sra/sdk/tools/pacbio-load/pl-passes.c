...
static rc_t passes_load_loop( ld_context *lctx, Passes_src *tab,
                              uint32_t *col_idx )
{
    rc_t rc = 0;
    KLogLevel tmp_lvl;
    ...
    rc = progress_chunk( lctx, total_passes );
    ...
    while( pos < total_passes && rc == 0 )
    {
        ...
        if ( rc == 0 )
        {
            ...
            for ( i = 0; i < block.n_read && rc == 0; ++i )
            {
                ...
                if ( rc == 0 )
                {
                    rc = passes_load_pass( lctx->cursor, &block, i, col_idx );
                    if ( rc == 0 )
                    {
...
static rc_t passes_loader( ld_context *lctx, const char * table_name,
                           bool cache_content )
{
    uint32_t col_idx[ passes_tab_count ];
    rc_t rc = add_columns( lctx->cursor, passes_tab_count, -1,
                           col_idx, passes_tab_names );
    if ( rc == 0 )
    {
        ...
        {
            Passes_src Passes;
            rc = open_Passes_src( lctx->hdf5_dir, &Passes,
                                  "PulseData/ConsensusBaseCalls/Passes",
                                  cache_content );
            if ( rc == 0 )
            {
                if ( check_Passes_extents( &Passes ) )
                    rc = passes_load_loop( lctx, &Passes, col_idx );
                else
...
rc_t load_passes( ld_context *lctx, bool cache_content, bool check_src_obj )
{
    rc_t rc = 0;
    if ( check_src_obj )
        rc = check_src_objects( lctx->hdf5_dir, passes_groups_to_check, 
                                passes_tables_to_check, false );
    if ( rc == 0 )
        rc = load_table( lctx, passes_schema_template, 
