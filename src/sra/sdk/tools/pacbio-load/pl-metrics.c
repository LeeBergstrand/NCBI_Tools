...
static rc_t metrics_load_loop( ld_context *lctx, Metrics_src *tab,
                               uint32_t *col_idx )
{
    rc_t rc = 0;
    KLogLevel tmp_lvl;
    ...
    rc = progress_chunk( lctx, total_rows );
        ...
        if ( rc == 0 )
        {
            ...
            for ( i = 0; i < block.n_read && rc == 0; ++i )
            {
                ...
                if ( rc == 0 )
                {
                    ...
                    rc = metrics_load( lctx->cursor, &block, i, col_idx );
                    if ( rc == 0 )
                    {
                        rc = progress_step( lctx );
...
static rc_t metrics_loader( ld_context *lctx, const char * table_name,
                            bool cache_content )
{
    uint32_t col_idx[ metrics_tab_count ];
    rc_t rc = add_columns( lctx->cursor, metrics_tab_count, -1,
                           col_idx, metrics_tab_names );
    if ( rc == 0 )
    {
        ...
        {
            Metrics_src Metrics;
            rc = open_Metrics_src( lctx->hdf5_dir, &Metrics,
                                   "PulseData/BaseCalls/ZMWMetrics",
                                   cache_content );
            if ( rc == 0 )
            {
                if ( check_Metrics_extents( &Metrics ) )
                    rc = metrics_load_loop( lctx, &Metrics, col_idx );
                else
...
rc_t load_metrics( ld_context *lctx, bool cache_content, bool check_src_obj )
{
    rc_t rc = 0;
    if ( check_src_obj )
        /* in case of metrics any combination of columns can be missing... */
        rc = check_src_objects( lctx->hdf5_dir, metrics_groups_to_check, 
                                NULL, false );

    if ( rc == 0 )
        rc = load_table( lctx, metrics_schema_template, 
