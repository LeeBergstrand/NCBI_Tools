                         ...
                         bool cache_content, bool supress_err_msg )
{
    ...
    if ( rc == 0 )
        rc = open_element( hdf5_dir, &tab->QualityValue, path, "QualityValue", 
                           ...
                           true, cache_content, supress_err_msg );
    if ( rc == 0 )
        rc = open_element( hdf5_dir, &tab->DeletionQV, path, "DeletionQV",
                           DELETION_QV_BITSIZE, DELETION_QV_COLS,
                           true, cache_content, supress_err_msg );
    if ( rc == 0 )
        rc = open_element( hdf5_dir, &tab->DeletionTag, path, "DeletionTag", 
                           DELETION_TAG_BITSIZE, DELETION_TAG_COLS,
                           true, cache_content, supress_err_msg );
    if ( rc == 0 )
        rc = open_element( hdf5_dir, &tab->InsertionQV, path, "InsertionQV", 
                           INSERTION_QV_BITSIZE, INSERTION_QV_COLS,
                           true, cache_content, supress_err_msg );
    if ( rc == 0 )
        rc = open_element( hdf5_dir, &tab->SubstitutionQV, path, "SubstitutionQV",
                           SUBSTITUTION_QV_BITZISE, SUBSTITUTION_QV_COLS,
                           true, cache_content, supress_err_msg );
    if ( rc == 0 )
        rc = open_element( hdf5_dir, &tab->SubstitutionTag, path, "SubstitutionTag",
