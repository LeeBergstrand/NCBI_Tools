                             ...
                             bool *have_high_quality )
{
    ...
    {   /* the type is unknown */
        rc = RC ( rcExe, rcNoTarg, rcLoading, rcData, rcInconsistent );
        LOGERR( klogErr, rc, "(type is unknown) in region-check()'" );
    }
    return rc;
}
                                ...
                                region_type_mapping *mapping )
{
    ...
    for ( idx = 0; idx < count && rc == 0; ++idx )
    {
        ...
        if ( rc == 0 )
            rc = rgn_type_string( name, idx, mapping );
    }
...
{
    if ( mapping == NULL )
        return RC( rcExe, rcNoTarg, rcLoading, rcParam, rcInvalid );
    OUTMSG(( "rgn-type-mapping->adapter   = %i\n", mapping->rgn_type_adapter ));
    OUTMSG(( "rgn-type-mapping->insert    = %i\n", mapping->rgn_type_insert ));
    OUTMSG(( "rgn-type-mapping->high_qual = %i\n", mapping->rgn_type_hq ));
    OUTMSG(( "rgn-type-mapping->globe_acc = %i\n", mapping->rgn_type_ga ));
    return 0;
}
