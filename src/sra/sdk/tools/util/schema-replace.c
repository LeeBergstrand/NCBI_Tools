...
{
    KDatabase *this_kdb;
    rc_t rc = KDBManagerOpenDBUpdate ( ctx->kdb_mgr, &this_kdb, csra );
    if ( rc == 0 )
    {
        ...
        if ( rc == 0 )
        {
            rc = KDatabaseOpenTableUpdate ( this_kdb, &this_tab, ctx->tab_name );
            if ( rc == 0 )
            {
            ...
            if ( rc == 0 )
            {
                ...
                if ( rc == 0 )
                {
                    ...
                    if ( rc ==  0 )
                    {
                        ...
                        if ( rc == 0 )
                        {
                            rc = VSchemaParseFile( ctx.schema, schema_name );
                            if ( rc == 0 )
                            {
