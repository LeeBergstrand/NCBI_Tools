...
static
char *rewrite_arg ( char *arg, bool should_free )
{
    char *result, *orig_arg = arg;
    bool has_drive = false;
    size_t i, dst_size;
    DWORD len;

    /* detect drive or full path */
    char rewrit [ MAX_PATH ];
    if ( arg [ 0 ] < 128 )
    {
        bool rewrite = false;
        ...
        if ( arg [ 0 ] == '\\' || arg [ 0 ] == '/' )
        {
            ...
            {
                /* check for cygdrive */
                if ( memcmp( arg, "/cygdrive/", sizeof "/cygdrive/" - sizeof "" ) == 0 )
        ...
        {
            /* incomplete path */
            len = GetFullPathNameA ( arg, sizeof rewrit / sizeof rewrit [ 0 ], rewrit, NULL );
    ...
    if ( result != NULL )
    {
        ...
        strcpy( result + offset, arg + offset );
...
static
char *rewrite_wide_arg ( const wchar_t *arg )
{
    char *utf8;
    bool has_drive = false;
...
int __cdecl wmain ( int argc, wchar_t *wargv [], wchar_t *envp [] )
{
    ...
    else
    {
        ...
        for ( i = 0; i < argc; ++ i )
        {
            argv [ i ] = rewrite_wide_arg ( wargv [ i ] );
            if ( argv [ i ] == NULL )
                break;
        }
...
int __cdecl main ( int argc, char *argv [], char *envp [] )
{
    ...
    /* rewrite any argument that looks like a path */
    for ( i = 0; i < argc; ++ i )
    {
        argv [ i ] = rewrite_arg ( argv [ i ], false );
        if ( argv [ i ] == NULL )
            break;
    }
