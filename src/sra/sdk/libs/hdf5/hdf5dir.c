    ...
    void *data, const char *path, va_list args )
{
    ...
    else
    {
        ...
        rc = KDirectoryOpenDirRead ( &self->dad, &sub, false, path );
        if ( rc == 0 )
        {
    ...
    const uint32_t type, const char * name, void * data )
{
    ...
    if ( self->h5root )
        psize = snprintf ( buffer, sizeof buffer, "/%s", name );
    else
        psize = snprintf ( buffer, sizeof buffer, "/%s%s", self->path, name );
    ...
    void *data, const char *path, va_list args )
{
    ...
    if ( rc == 0 )
    {
        ...
        for ( idx = 0; idx < count && rc == 0; ++idx )
        {
            ...
            if ( rc == 0 )
            {
                uint32_t type = KDirectoryPathType ( &self->dad, name );
                switch ( type )
                {
                case kptDataset  :
                case kptDatatype :
                case kptDir : rc = HDF5DirVisitcb( self, f, type, name, data ); break;
                }
                if ( recurse && type == kptDir )
                {
                    const KDirectory *sub;
                    rc = KDirectoryOpenDirRead ( &self->dad, &sub, false, name );
                    if ( rc == 0 )
                    {
                        KDirectoryVisit( sub, true, f, data, NULL );
                        KDirectoryRelease( sub );
                    }
                }
            }
    ...
    const char *path, va_list args )
{
    ...
    H5O_info_t obj_info;
    char buffer[ PATH_MAX ];
    size_t psize = vsnprintf ( buffer, sizeof buffer, path, args );
    if ( psize < 0 )
        return kptBadPath;
    h5e = H5Oget_info_by_name( self->hdf5_handle, buffer, &obj_info, H5P_DEFAULT );
    if ( h5e >= 0 )
    {
    ...
    struct KFile const **f, const char *path, va_list args )
{
    ...
    *f = NULL;
    ...
    path_type = HDF5DirPathType ( self, path, args );
    if ( path_type != kptDataset )
        return RC( rcFS, rcDirectory, rcAccessing, rcInterface, rcUnsupported );

    psize = vsnprintf ( buffer, sizeof buffer, path, args );
    if ( psize < 0 )
        return RC( rcFS, rcDirectory, rcAccessing, rcInterface, rcUnsupported );
    ...
    const KDirectory **sub, bool chroot, const char *path, va_list args )
{
    ...
    *sub = NULL;
    buffer_size = vsnprintf ( buffer, sizeof buffer, path, args );
    if ( buffer_size < 0 )
        return RC ( rcFS, rcDirectory, rcOpening, rcPath, rcExcessive );
 
    type = KDirectoryPathType ( &self->dad, buffer );
    switch( type )
    {
    case kptNotFound :
                                     ...
                                     bool absolute, const char *path )
{
    ...
    * hdf5_dir = NULL;
    ...
    rc = KDirectoryResolvePath ( self, absolute, resolved, sizeof resolved, path );
    if ( rc != 0 ) return rc;
    path_size = string_size( resolved );

    path_type = KDirectoryPathType ( self, resolved );
    switch ( path_type )
    {
    case kptNotFound :
