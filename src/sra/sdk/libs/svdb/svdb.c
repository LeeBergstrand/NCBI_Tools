...
static int svdb_strcmp( const char *a, const char *b )
{
    ...
    return string_cmp ( a, sa, b, sb, ( sa > sb ) ? sa : sb );
}
...
MOD_EXPORT const char * CC svdb_accession_2_path( const char * accession )
{
    ...
    if ( accession != NULL && accession[0] != 0 )
    {
        if ( strchr ( accession, '/' ) == NULL )
        {
            ...
            if ( rc != 0 )
            {
                LOGERR( klogInt, rc, "KDirectoryNativeDir() failed in svdb_accession_2_path()" );
                svdb_set_last_err( "KDirectoryNativeDir() failed in svdb_accession_2_path()" );
            }
            else
            {
                SRAPath *sra_path;

                rc = SRAPathMake( &sra_path, dir );
                if ( rc != 0 )
                {
                    LOGERR( klogInt, rc, "SRAPathMake() failed in svdb_accession_2_path()" );
                    svdb_set_last_err( "SRAPathMake() failed in svdb_accession_2_path()" );
                }
                else
                {
                    rc = SRAPathFind( sra_path, 
                                      accession,
                                      accession_2_path_buffer,
                                      sizeof accession_2_path_buffer );
                    if ( rc == 0 )
                    {
                        res = accession_2_path_buffer;
                    }
                    else
                    {
                        svdb_set_last_err( "SRAPathFind() failed in svdb_accession_2_path()" );
                    }
...
static p_svdb_type svdb_make_type( p_svdb_col col, const char * name )
{
    ...
    else
    {
        ...
        rc = VSchemaResolveTypedecl ( col->tab->conn->schema, &res->typedecl, name );
        if ( rc != 0 )
        {
            ...
            if ( rc == 0 )
            {
                ...
                for ( i = 0; rc == 0 && i < count; ++ i )
                {
                    ...
                    if ( rc == 0 )
                    {
                        ...
                        rc = KMDataNodeOpenNodeRead ( node, &child_node, node_path );
                        if ( rc == 0 )
                        {
    ...
    if ( rc == 0 )
    {
        ...
        if ( rc == 0 )
        {
            ...
            for ( i = 0; i < n && rc == 0; ++i )
            {
                ...
                {
                    const VTable * tab;
                    rc = VDatabaseOpenTableRead( pself->db, &tab, tbl_name );
                    if ( rc == 0 )
                    {
        ...
        else
        {
            ...
            if ( rc == 0 )
            {
                ...
                else
                {
                    ...
                    rc = VDBManagerOpenDBRead( pself->mgr, &pself->db, pself->schema, path );
                    if ( rc == 0 )
                    {
                        pself->is_db = 1;
                    ...
                    {
                        const VTable * tab;
                        rc = VDBManagerOpenTableRead( pself->mgr, &tab, pself->schema, path );
                        if ( rc == 0 )
                        {
...
static rc_t svdb_print_hex( char * dst, int dstlen, char * src, int srclen,
                            int trim, size_t * num_writ )
{
    rc_t rc = 0;
    int periods = 1;
    int i, n = ( dstlen / 3 );
...
static int svdb_add_to_cursor( p_svdb_tab tab )
{
    ...
    for ( i = 0; i < n && rc == 0; ++i )
    {
        ...
        if ( col != NULL && col->enabled )
        {
            ...
            {
                rc = VCursorAddColumn ( tab->cursor, &col->cur_idx, col->name );
            }
            if ( rc != 0 )
            {
...
{
    unsigned long long int res = 0xFFFFFFFFFFFFFFFF;
    p_svdb_col col = svdb_get_col( self, tab_id, selection, col_id );
    if ( col != NULL && row < col->tab->range && col->open && pattern != NULL )
    {
        ...
        while( ( rc == 0 ) && ( res == 0xFFFFFFFFFFFFFFFF ) &&
               ( search_row < col->tab->range ) && ( chunk < chunksize ) )
        {
            data_src data;
        ...
        if ( ( chunk == chunksize )&&( res == 0xFFFFFFFFFFFFFFFF ) )
        {
