...
export MARCH=$($1/build/mac.arch.sh)
if [ "$MARCH" = "x86_64" ]
then
    export ARCH_BITS=64
else
    echo "not a 64 bit machine" >&2
fi
...
export VDB_CONFIG=/net/traces01.be-md.ncbi.nlm.nih.gov/trace_software/vdb/config:/net/traces01.be-md.ncbi.nlm.nih.gov/trace_software/vdb/mac/config
echo "LD_LIBRARY_PATH=${LD_LIBRARY_PATH}" >&2
cd $1

make NOREBUILD_LINKS=1 local dynamic debug clean config std 
if [ "$?" != "0" ] ; then
    exit 3
fi
