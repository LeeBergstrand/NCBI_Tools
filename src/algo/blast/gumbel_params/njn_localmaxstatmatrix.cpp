/* $Id: njn_localmaxstatmatrix.cpp 319891 2011-07-26 15:10:40Z boratyng $
* ===========================================================================
*
...
void LocalMaxStatMatrix::init (size_t dimMatrix_, size_t dimMatrix2_)
{
    ...
    {
        d_scoreMatrix_p = MemUtil::newMatrix <Int4> (dimMatrix_, dimMatrix2_); 
...
void LocalMaxStatMatrix::free ()
{
    if (getDimMatrix () > 0 && getDimMatrix2 () > 0) 
    {
        MemUtil::deleteMatrix <Int4> (d_scoreMatrix_p, getDimMatrix (), getDimMatrix2 ());
        d_scoreMatrix_p = 0;
...
const Int4 *const *scoreMatrix_, // score matrix [0...dimMatrix_)[0...dimMatrix_)
...
size_t dimMatrix2_) // #(distinct letters) in the second alphabet         
{
    ...
    {
        memcpy (d_scoreMatrix_p [i], scoreMatrix_ [i], sizeof (Int4) * getDimMatrix2 ());
    }
